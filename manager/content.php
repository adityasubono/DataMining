...
if ($_GET[module]=='home'){
	echo "<h3>Aplikasi Data Mining C.45 </h3>
...
elseif ($_GET[module]=='inputdataa'){
		  $sql=mysql_query("SELECT * FROM data_survey WHERE umur='$_GET[jenis]' AND status='ya'");
		  $sql2=mysql_query("SELECT * FROM data_survey WHERE umur='$_GET[jenis]' AND status='tidak'");
		  $sql3=mysql_query("SELECT * FROM data_survey WHERE umur='$_GET[jenis]'");
...
elseif ($_GET[module]=='editdata'){
  ...
  $e=mysql_fetch_array($sql);
	 echo "<h2>Edit Data Pinjaman</h2><hr><br/><br/>
		  ...
		  <tr><td width='165px'>No Rekening</td>   		 <td>:</td> <td> <input type=text value='$e[kode_barang]' name='kode_barang' style='width:99%'></td></tr>
		  <tr><td>Umur</td>    	 <td>:</td> <td>
				<select style='width:100%; margin-bottom:5px;' name='jenis_barang'>
				<option value='$e[umur]' selected> $e[umur] </option>";
				$tampil=mysql_query("SELECT * FROM atribut WHERE atribut='umur'");
				while($r=mysql_fetch_array($tampil)){
				echo "<option value='$r[nilai_atribut]'>$r[nilai_atribut]</option>";
				}
		  echo " </select></td></tr>
		  <tr><td>Penghasilan</td>    	 <td>:</td> <td>
				<select style='width:100%; margin-bottom:5px;' name='merek'>
				<option value='$e[penghasilan]' selected> $e[penghasilan] </option>";
				$tampil=mysql_query("SELECT * FROM atribut WHERE atribut='penghasilan'");
				while($r=mysql_fetch_array($tampil)){
				echo "<option value='$r[nilai_atribut]'>$r[nilai_atribut]</option>";
				}
		  echo " </select></td></tr>
		  <tr><td>Pekerjaan</td>    	 <td>:</td> <td>
				...
				<option value='$e[pekerjaan]' selected> $e[pekerjaan] </option>";
				$tampil=mysql_query("SELECT * FROM atribut WHERE atribut='pekerjaan'");
				while($r=mysql_fetch_array($tampil)){
				echo "<option value='$r[nilai_atribut]'>$r[nilai_atribut]</option>";
				}
		  echo " </select></td></tr>
		  <tr><td>Pinjaman</td>     <td>:</td> <td><input type=text value='$e[harga]' name='harga' style='width:99%'></td></tr>
		  <tr><td>Status</td>     <td>:</td> <td> 
				<select style='width:100%; margin-bottom:5px;' name='status'>
...
elseif ($_GET[module]=='simpanedit'){
    mysql_query("UPDATE data_survey SET kode_barang  = '$_POST[kode_barang]',
									   umur = '$_POST[jenis_barang]', 
									   penghasilan	   = '$_POST[merek]',
									   pekerjaan       = '$_POST[tahun]',
									   harga       = '$_POST[harga]',
									   status	   = '$_POST[status]'
								 WHERE id   = '$_POST[id]'");
